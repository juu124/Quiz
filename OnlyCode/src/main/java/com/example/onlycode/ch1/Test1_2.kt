...
//    printOrderFlow(orders.asFlow())
    ...
    orders.asFlow()
        ...
        .collect {
            println("//Order #${it.orderId} completed - Priority shipping (Total: ${String.format("%, d", it.totalAmount)} KRW)")
        }
}

// suspend 함수로 정의하여 flow를 collect 할 수 있게 함
//suspend fun printOrderFlow(orderList: List<Order>) {
//    val output = "//Order #${orderList.orderId}: ${order.items} - ${order.totalAmount} KRW [${order.status}]$priorityText"
//    println(output)
//}